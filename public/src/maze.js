...
export function buildWalls(scene, maze) {
  // Wall material: use the provided dark brick pattern as a tiled texture
  // with a teal base color (#008c8c). If you move to production, download
  // and re-host this asset instead of hotlinking to transparenttextures.
  const loader = new THREE.TextureLoader();
  const wallTex = loader.load(
    // This is intended for prototyping; consider re-hosting for production.
    "https://www.transparenttextures.com/patterns/dark-brick-wall.png"
  );
  wallTex.wrapS = wallTex.wrapT = THREE.RepeatWrapping;
  // Tune repeat so the brick pattern tiles reasonably across each wall cell.
  // These values are conservative; tweak if you want larger/smaller bricks.
  wallTex.repeat.set(1, 1);

  const mat = new THREE.MeshStandardMaterial({
    color: 0x008c8c, // teal base to match the requested background-color
    map: wallTex,
    roughness: 0.6,
    metalness: 0.05,
  });
  ...
  for (let y = 0; y < MAZE.H; y++) {
    for (let x = 0; x < MAZE.W; x++) {
      if (maze[y][x] === 0) {
        ...
        group.add(mesh);
        const half = MAZE.CELL / 2;
        // Capture grid coords and mesh to make it easy to attach decals/signs
        walls.push({
          x,
          y,
          mesh,
